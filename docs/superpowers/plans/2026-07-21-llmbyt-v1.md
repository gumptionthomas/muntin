# llmbyt v1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship `llmbyt` v1 — a harness-agnostic Python CLI that renders 64×32 frames and pushes them to a Tidbyt as ephemeral interrupts, with a preview loop an agent can close by itself.
...
- **Loud errors.** Never swallow, never fall back to a stale artifact. Every custom error derives from `llmbyt.errors.LlmbytError` and names the constraint, the measured violation, and the fix.
- **The API token is never printed** — not in logs, not in errors, not in tracebacks.
- Push always uses `background: false` and `installationID: "llmbyt"`. Never create, delete, or reorder other installations.
- All source lives under `src/llmbyt/`; all tests under `tests/`.
...
**Files:**
- Create: `pyproject.toml`, `.gitignore`, `src/llmbyt/__init__.py`, `src/llmbyt/errors.py`, `src/llmbyt/font.py`
- Create (vendored): `src/llmbyt/fonts/tom-thumb.bdf`, `src/llmbyt/fonts/spleen-5x8.bdf`, `src/llmbyt/fonts/LICENSES.md`
- Test: `tests/test_font.py`

**Interfaces:**
- Consumes: nothing.
- Produces: `llmbyt.errors.LlmbytError` (base for all later errors). `llmbyt.font.Font` with attributes `name: str`, `char_w: int`, `char_h: int`, and methods `text_width(s: str) -> int`, `draw(draw, xy: tuple[int,int], s: str, fill: tuple[int,int,int]) -> int`. Module functions `load(name: str = DEFAULT) -> Font`, `available() -> list[str]`, constant `DEFAULT = "tom-thumb"`.
...
```bash
cd /home/gumptionthomas/Development/llmbyt
mkdir -p src/llmbyt/fonts tests
```
...
```bash
cd /home/gumptionthomas/Development/llmbyt
cp /home/gumptionthomas/Development/claude-desktop-buddy/linux-bridge/src/familiar/fonts/tom-thumb.bdf src/llmbyt/fonts/
curl -sf https://raw.githubusercontent.com/fcambus/spleen/master/spleen-5x8.bdf -o src/llmbyt/fonts/spleen-5x8.bdf
wc -l src/llmbyt/fonts/*.bdf
```
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest tests/test_font.py -q
```

Expected: collection error — `ModuleNotFoundError: No module named 'llmbyt.font'`.
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest tests/test_canvas.py -q
```

Expected: collection error — `ModuleNotFoundError: No module named 'llmbyt.canvas'`.

- [ ] **Step 3: Implement the canvas**

`src/llmbyt/canvas.py`:
...
**Files:**
- Create: `src/llmbyt/encode.py`
- Test: `tests/test_encode.py`

**Interfaces:**
- Consumes: `llmbyt.canvas.W/H`, `llmbyt.errors.LlmbytError`.
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest tests/test_encode.py -q
```

Expected: collection error — `ModuleNotFoundError: No module named 'llmbyt.encode'`.

- [ ] **Step 3: Implement the encoder**

`src/llmbyt/encode.py`:
...
**Files:**
- Create: `src/llmbyt/preview.py`, `tests/golden.py`
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest tests/test_preview.py -q
```

Expected: collection error — `ModuleNotFoundError: No module named 'llmbyt.preview'`.

- [ ] **Step 4: Implement the preview writer**

`src/llmbyt/preview.py`:
...
**Files:**
- Create: `src/llmbyt/scene.py`
- Test: `tests/test_scene_layout.py`

**Interfaces:**
- Consumes: `llmbyt.canvas.Canvas/W/H/WHITE`, `llmbyt.font`, `llmbyt.errors.LlmbytError`.
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest tests/test_scene_layout.py -q
```

Expected: collection error — `ModuleNotFoundError: No module named 'llmbyt.scene'`.
...
**Files:**
- Modify: `src/llmbyt/scene.py` (append; do not alter Task 5's classes)
- Test: `tests/test_scene_nodes.py`, `tests/test_scene_render.py`

**Interfaces:**
- Consumes: everything from Task 5, plus `llmbyt.canvas.Canvas/W/H`, `llmbyt.encode.max_frames`.
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest tests/test_scene_nodes.py tests/test_scene_render.py -q
```

Expected: failures — `AttributeError: module 'llmbyt.scene' has no attribute 'Marquee'`.

- [ ] **Step 4: Append the remaining nodes and the renderer**

Append to `src/llmbyt/scene.py`:
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest -q
```
...
**Files:**
- Create: `src/llmbyt/device.py`
- Test: `tests/test_device.py`

**Interfaces:**
- Consumes: `llmbyt.errors.LlmbytError`.
- Produces: `ConfigError`, `PushError`, `PUSH_URL`, `INSTALLATION_ID = "llmbyt"`, `CONFIG_PATH: pathlib.Path`. Dataclass `Config(device_id: str, api_token: str)` with `redact(text: str) -> str`. Functions `load_config(env=None, path=None) -> Config`, `save_config(cfg, path=None) -> pathlib.Path`, `push(webp: bytes, cfg: Config, *, poster=None) -> None`.
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest tests/test_device.py -q
```

Expected: collection error — `ModuleNotFoundError: No module named 'llmbyt.device'`.

- [ ] **Step 3: Implement config and push**

`src/llmbyt/device.py`:
...
```bash
cd /home/gumptionthomas/Development/llmbyt && grep -rn "urlopen\|http" tests/ || echo "clean"
```
...
**Files:**
- Create: `src/llmbyt/runner.py`
- Test: `tests/test_runner.py`

**Interfaces:**
- Consumes: `llmbyt.scene.Node/render_scene`, `llmbyt.canvas.W/H`, `llmbyt.encode.FRAME_MS_DEFAULT`, `llmbyt.errors.LlmbytError`.
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest tests/test_runner.py -q
```

Expected: collection error — `ModuleNotFoundError: No module named 'llmbyt.runner'`.

- [ ] **Step 3: Implement the runner**

`src/llmbyt/runner.py`:
...
`--help` is a teaching surface. The top-level epilog states the display size, the duration cap, and the three accepted `render()` return types, so an agent that runs `llmbyt --help` and nothing else still knows the contract.

`main` returns an exit code and prints `LlmbytError` messages to stderr without a traceback — the message *is* the interface. Any other exception keeps its traceback, because that is a bug in llmbyt rather than user error.
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest tests/test_cli.py -q
```

Expected: collection error — `ModuleNotFoundError: No module named 'llmbyt.cli'`.
...
cd /home/gumptionthomas/Development/llmbyt
uv run llmbyt --help
printf 'from llmbyt import scene as sc\ndef render():\n    return sc.Text("hello")\n' > /tmp/hello.py
uv run llmbyt preview /tmp/hello.py -o /tmp/hello.png
```
...
```bash
cd /home/gumptionthomas/Development/llmbyt && uv run pytest tests/test_examples.py -q
```
...
```bash
cd /home/gumptionthomas/Development/llmbyt
for f in examples/*.py; do uv run llmbyt preview "$f" -o "/tmp/llmbyt-$(basename "$f" .py)"; done
ls -la /tmp/llmbyt-*
```
